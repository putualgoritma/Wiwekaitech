...
@router.post("/product-image", dependencies=[Depends(require_role("admin", "editor"))])
async def upload_product_image(
    ...
    # Define upload path
    upload_dir = Path(__file__).parent.parent / "static" / "uploads" / "products"
    upload_dir.mkdir(parents=True, exist_ok=True)
    file_path = upload_dir / filename
...
@router.post("/tutorial-image", dependencies=[Depends(require_role("admin", "editor"))])
async def upload_tutorial_image(
    file: UploadFile = File(...),
    current_user: User = Depends(get_current_user)
):
    """
    Upload a tutorial image.
    Accessible to: admin, editor
    ...
    upload_dir = Path(__file__).parent.parent / "static" / "uploads" / "tutorials"
    upload_dir.mkdir(parents=True, exist_ok=True)
    file_path = upload_dir / filename
    ...
    image_url = f"/static/uploads/tutorials/{filename}"
